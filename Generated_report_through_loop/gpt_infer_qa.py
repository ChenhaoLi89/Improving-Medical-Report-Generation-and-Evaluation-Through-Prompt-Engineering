        ...
        response = openai.ChatCompletion.create(
            ...
            messages=[
                ...
                {"role": "user", "content": [
                    ...
                    {"type": "text", "text": prompt + "The report should be a paragraph of about 200 words."}
                ]}
            ],
            max_tokens=1000, 
            temperature=0.7 
...
for i in tqdm(range(15)):
    ...
    while generated_report == None:
        generated_report = gpt4o_infer(encoded_image, prompt)
    print(generated_report)
    
    score_reason = gpt4o_eval(generated_report, original_report, question_txt)
    while score_reason == None:
        score_reason = gpt4o_eval(generated_report, original_report, question_txt)
    print(score_reason)
    
    new_prompt = gpt4o_reflect(encoded_image, generated_report, score_reason, prompt)
    while new_prompt == None:
        new_prompt = gpt4o_reflect(encoded_image, generated_report, score_reason, prompt)
    print(new_prompt)
    ...
    if i % 2 == 0 and i > 0:
        ...
        L_image_url.sort()
        for j in range(len(L_image_url)):
            image_url = L_image_url[j]
            with open(image_url, "rb") as image_file:
                encoded = base64.b64encode(image_file.read()).decode("utf-8")
            answer = gpt4o_infer(encoded, new_prompt)
            
            while answer == None:
                answer = gpt4o_infer(encoded, new_prompt)
            
            save_folder = "./Baseline_result_qa_gpt_new/llama_infer"+str(i)+"/" 
            path = image_url
